...
let currentHistoryTab = 'Tugas Kuliah'; // Default tab riwayat
...
async function loadTodaySchedule() {
    ...
    finalMatkuls.forEach(m => {
        ...
        combinedSchedule.push({ id: m.id, name: `Matkul: ${m.name}`, time: time, color: 'var(--color-matkul)', details: `R: ${ruang} | Dosen: ${dosen}` });
    });

    // 2. Tugas & Acara (Yang belum selesai)
    let todaysTasks = savedTasks.filter(t => t.date === todayStr && t.status !== 'done');
    ...
    try {
        ...
        combinedSchedule.push({ name: 'Maghrib (Offline)', time: '17:45', color: '#9BBAD4', details: 'Rutinitas' });
    }
...
// --- LOGIKA KALENDER & OVERRIDE MATKUL ---
if(document.getElementById('calendar-grid')) {
    renderMonthCalendar();
    renderHistory('Tugas Kuliah'); // Render awal riwayat
...
function renderMonthCalendar() {
    ...
    for(let i = 1; i <= daysInMonth; i++) {
        ...
        // Logika Titik Matkul (Cek Override)
        ...
        if(matkulsToday.length > 0 || movedIn.length > 0) {
            dotsHtml += `<div class="dot" style="background-color: var(--color-matkul);"></div>`;
        }
        
        // Titik Tugas/Acara (Hanya yang belum selesai)
        savedTasks.filter(t => t.date === currentLoopDate && t.status !== 'done').forEach(t => {
            dotsHtml += `<div class="dot" style="background-color: ${t.color || 'var(--color-tugas)'};"></div>`;
        });
        
        grid.innerHTML += `<div class="day-cell" onclick="showDayDetails('${currentLoopDate}', '${loopDayName}')">
            <span class="day-number">${i}</span><div class="event-dots">${dotsHtml}</div>
        </div>`;
    }
}

// Tampilkan Pop Up Detail Hari
function showDayDetails(dateStr, dayName) {
    ...
    // Gabungkan Matkul normal & hasil pindahan
    ...
    let movedIn = overrides.filter(o => o.newDate === dateStr).map(o => {
        let orig = savedMatkul.find(m => m.id === o.matkulId);
        return orig ? { ...orig, ...o, isOverride: true } : null;
    }).filter(x => x);
    let finalMatkuls = [...matkulsToday, ...movedIn];
    ...
    finalMatkuls.forEach(m => {
        ...
        let ruang = m.isOverride ? m.newRuangan : m.ruangan;
        let dosen = m.isOverride ? m.newDosen : m.dosen;
        let origDate = m.isOverride ? m.originalDate : dateStr; // Catat akar tanggal aslinya
        ...
        listHtml += `<div style="padding: 10px; border-left: 3px solid var(--color-matkul); margin-bottom: 8px; background: var(--bg-main); border-radius: 5px;">
            <button class="edit-btn" onclick="openRescheduleModal(${m.id}, '${origDate}', '${dateStr}')">✏️</button>
            <strong>${time} - ${m.name}</strong> <br>
            <small>Ruang: ${ruang} | Dosen: ${dosen}</small>
        </div>`;
    });
    ...
    tasksThisDay.forEach(t => {
        listHtml += `<div style="padding: 10px; border-left: 3px solid ${t.color || 'var(--color-tugas)'}; margin-bottom: 8px; background: var(--bg-main); border-radius: 5px;">
            <button class="edit-btn" onclick="markTaskDone(${t.id}, '${dateStr}', '${dayName}')" style="color: #A3D9A5;">✔️</button>
...
// Buka Modal Edit Khusus Matkul
function openRescheduleModal(matkulId, origDate, activeDate) {
    closeModal('day-detail-modal');
    let savedMatkul = JSON.parse(localStorage.getItem('nalaMatkul')) || [];
    let overrides = JSON.parse(localStorage.getItem('nalaOverrides')) || [];
    
    let original = savedMatkul.find(m => m.id === matkulId);
    let existing = overrides.find(o => o.matkulId === matkulId && o.originalDate === origDate);
    
    document.getElementById('res-matkulId').value = matkulId;
    document.getElementById('res-originalDate').value = origDate; 
    
    document.getElementById('res-date').value = existing ? existing.newDate : activeDate;
    document.getElementById('res-time').value = existing ? existing.newTime : original.jamMulai;
    document.getElementById('res-ruang').value = existing ? existing.newRuangan : original.ruangan;
    document.getElementById('res-dosen').value = existing ? existing.newDosen : original.dosen;
    
    document.getElementById('reschedule-modal').style.display = 'flex';
}

// Simpan Perubahan Jadwal Matkul
function saveReschedule() {
    const mId = parseInt(document.getElementById('res-matkulId').value);
    const origDate = document.getElementById('res-originalDate').value;
    
    let overrides = JSON.parse(localStorage.getItem('nalaOverrides')) || [];
    // Buang override lama di tanggal akar yang sama
    overrides = overrides.filter(o => !(o.matkulId === mId && o.originalDate === origDate));
    
    // Masukkan override baru
    overrides.push({
        matkulId: mId,
        originalDate: origDate,
        newDate: document.getElementById('res-date').value,
        newTime: document.getElementById('res-time').value,
        newRuangan: document.getElementById('res-ruang').value,
        newDosen: document.getElementById('res-dosen').value
    });
...
function markTaskDone(id, dateStr, dayName) {
    ...
    if(idx !== -1) {
        ...
        showDayDetails(dateStr, dayName); // Refresh popup
        renderMonthCalendar(); // Hilangkan dari titik kalender
        renderHistory(currentHistoryTab); // Munculkan di riwayat
    }
}

// Tampilkan Riwayat Selesai
function renderHistory(tabName) {
    currentHistoryTab = tabName;
    document.getElementById('tab-tk').classList.toggle('active', tabName === 'Tugas Kuliah');
    document.getElementById('tab-tb').classList.toggle('active', tabName === 'Tugas');
    
    let tasks = JSON.parse(localStorage.getItem('nalaTasks')) || [];
    let doneTasks = tasks.filter(t => t.status === 'done' && t.category === tabName);
    
    let list = document.getElementById('history-list');
    list.innerHTML = '';
    
    if(doneTasks.length === 0) {
        list.innerHTML = '<p style="color:var(--text-muted); font-size:14px; text-align:center;">Belum ada riwayat selesai.</p>';
        return;
    }
    
    doneTasks.sort((a,b) => b.date.localeCompare(a.date)); // Terbaru di atas
    doneTasks.forEach(t => {
        list.innerHTML += `<div class="history-card">
            ...
            <span style="color:#A3D9A5;">Selesai ✔️</span>
        </div>`;
    });
}

// --- LOGIKA SIMPAN TUGAS/ACARA ---
function openModal(category) { /* ... (Logika Sama Persis Seperti Sebelumnya) ... */
...
function openEditTask(id, category) { /* ... (Logika Sama Persis) ... */
...
function saveNewTask() { /* ... (Logika Sama Persis) ... */
    ...
    localStorage.setItem('nalaTasks', JSON.stringify(savedTasks)); closeModal('insert-modal');
    if(document.getElementById('schedule-list')) loadTodaySchedule();
    if(document.getElementById('calendar-grid')) { renderMonthCalendar(); renderHistory(currentHistoryTab); }
}
...
function saveMatkul() { /* ... (Logika Sama Persis) ... */
...
function renderMatkulList() { /* ... (Logika Sama Persis) ... */
    const list = document.getElementById('matkul-list'); list.innerHTML = ''; let savedMatkul = JSON.parse(localStorage.getItem('nalaMatkul')) || [];
...
function editMatkul(id) { /* ... (Logika Sama Persis) ... */
